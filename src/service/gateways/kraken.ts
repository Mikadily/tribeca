...
interface KrakenOrderBook {
    bids: KrakenMarketLevel[];
    asks: KrakenMarketLevel[];
}
...
class KrakenMarketDataGateway implements Interfaces.IMarketDataGateway {
    ConnectChanged = new Utils.Evt<Models.ConnectivityStatus>();

    private _since: number = null;
    MarketTrade = new Utils.Evt<Models.GatewayMarketTrade>();
    private onTrades = (trades: Models.Timestamped<KrakenMarketTrade[]>) => {
    ...
    private downloadMarketTrades = () => {
        ...
        this._http
            .get<KrakenMarketTrade[]>("trades/" + this._symbolProvider.symbol, qs)
            .then(this.onTrades)
            .done();
    };

    private static ConvertToMarketSide(level: KrakenMarketLevel): Models.MarketSide {
        return new Models.MarketSide(parseFloat(level.price), parseFloat(level.amount));
    }

    private static ConvertToMarketSides(level: KrakenMarketLevel[]): Models.MarketSide[] {
        return _.map(level, KrakenMarketDataGateway.ConvertToMarketSide);
    }

    MarketData = new Utils.Evt<Models.Market>();
    private onMarketData = (book: Models.Timestamped<KrakenOrderBook>) => {
        var bids = KrakenMarketDataGateway.ConvertToMarketSides(book.data.bids);
        var asks = KrakenMarketDataGateway.ConvertToMarketSides(book.data.asks);
        this.MarketData.trigger(new Models.Market(bids, asks, book.time));
    };

    private downloadMarketData = () => {
        this._http
            .get<KrakenOrderBook>("book/" + this._symbolProvider.symbol, { limit_bids: 5, limit_asks: 5 })
    ...
    constructor(
        ...
        private _http: KrakenHttp,
        private _symbolProvider: KrakenSymbolProvider) {
...
interface KrakenNewOrderResponse extends RejectableResponse {
    order_id: string;
}

interface KrakenCancelOrderRequest {
    order_id: string;
}

interface KrakenCancelReplaceOrderRequest extends KrakenNewOrderRequest {
    order_id: string;
}

interface KrakenCancelReplaceOrderResponse extends KrakenCancelOrderRequest, RejectableResponse { }

interface KrakenOrderStatusRequest {
    order_id: string;
}

interface KrakenMyTradesRequest {
    symbol: string;
    timestamp: number;
}

interface KrakenMyTradesResponse extends RejectableResponse {
    price: string;
    amount: string;
    timestamp: number;
    exchange: string;
...
interface KrakenOrderStatusResponse extends RejectableResponse {
    symbol: string;
    exchange: string; // bitstamp or 
    price: number;
...
class KrakenOrderEntryGateway implements Interfaces.IOrderEntryGateway {
    ...
    private convertToOrderRequest = (order: Models.Order): KrakenNewOrderRequest => {
        return {
    ...
    sendOrder = (order: Models.BrokeredOrder): Models.OrderGatewayActionReport => {
        ...
        this._http
            .post<KrakenNewOrderRequest, KrakenNewOrderResponse>("order/new", req)
            .then(resp => {
    ...
    cancelOrder = (cancel: Models.BrokeredCancel): Models.OrderGatewayActionReport => {
        ...
        this._http
            .post<KrakenCancelOrderRequest, any>("order/cancel", req)
            .then(resp => {
                if (typeof resp.data.message !== "undefined") {
                    this.OrderUpdate.trigger({
                        orderStatus: Models.OrderStatus.Rejected,
    ...
    private downloadOrderStatuses = () => {
        ...
        this._http
            .post<KrakenMyTradesRequest, KrakenMyTradesResponse[]>("mytrades", tradesReq)
            .then(resps => {
                _.forEach(resps.data, t => {

                    this._http
                        .post<KrakenOrderStatusRequest, KrakenOrderStatusResponse>("order/status", { order_id: t.order_id })
                        .then(r => {
                            ...
                            this.OrderUpdate.trigger({
                                ...
                                orderStatus: KrakenOrderEntryGateway.GetOrderStatus(r.data),
    ...
    private static GetOrderStatus(r: KrakenOrderStatusResponse) {
    ...
    private _log: Utils.Logger = Utils.log("tribeca:gateway:KrakenOE");
...
class KrakenHttp {
    ...
    // Kraken seems to have a race condition where nonces are processed out of order when rapidly placing orders
    ...
    private _log: Utils.Logger = Utils.log("tribeca:gateway:KrakenHTTP");
    ...
    constructor(config: Config.IConfigProvider, private _monitor: RateLimitMonitor) {
        this._baseUrl = config.GetString("KrakenHttpUrl")
        this._apiKey = config.GetString("KrakenKey");
        this._secret = config.GetString("KrakenSecret");
...
interface KrakenPositionResponseItem {
    type: string;
    currency: string;
    amount: string;
    available: string;
...
class KrakenPositionGateway implements Interfaces.IPositionGateway {
    PositionUpdate = new Utils.Evt<Models.CurrencyPosition>();

    private onRefreshPositions = () => {
        this._http.post<{}, KrakenPositionResponseItem[]>("balances", {}).then(res => {
    ...
    private _log: Utils.Logger = Utils.log("tribeca:gateway:KrakenPG");
    constructor(timeProvider: Utils.ITimeProvider, private _http: KrakenHttp) {
        timeProvider.setInterval(this.onRefreshPositions, moment.duration(15, "seconds"));
        this.onRefreshPositions();
    }
}

class KrakenBaseGateway implements Interfaces.IExchangeDetailsGateway {
    ...
    public get supportedCurrencyPairs() {
        return KrakenBaseGateway.AllPairs;
    }
}
...
export class Kraken extends Interfaces.CombinedGateway {
    constructor(timeProvider: Utils.ITimeProvider, config: Config.IConfigProvider, pair: Models.CurrencyPair) {
        var symbol = new KrakenSymbolProvider(pair);
        var monitor = new RateLimitMonitor(60, moment.duration(1, "minutes"));
        var http = new KrakenHttp(config, monitor);

        var orderGateway = config.GetString("KrakenOrderDestination") == "Kraken"
            ? <Interfaces.IOrderEntryGateway>new KrakenOrderEntryGateway(timeProvider, http, symbol)
            : new NullGateway.NullOrderGateway();

        super(
            new KrakenMarketDataGateway(timeProvider, http, symbol),
            orderGateway,
            new KrakenPositionGateway(timeProvider, http),
            new KrakenBaseGateway());
    }
}
